<?php 
		...
		$sql.=" AND (serial_number !='' AND product_number!='' AND health='good')";
		//$sql.=" AND (health='good')";
...
";
		...
		while($row=mysqli_fetch_array($rows_of_sid, MYSQLI_ASSOC)){
			...
			$sql="SELECT item_id,make,model,product_number,serial_number,availablity FROM item WHERE ";
			$x=0;
			foreach($item_type as $item ){
				if($x==0)
					$sql.=" ( ";
			...
			$sql.=" AND (serial_number !='' AND product_number !='' ) AND health='good' AND availablity!='issued'";
...
"				<tr id='item_row_{$i}_{$j}' style='display:none'>
				...
				$table.=">{$adv_row['availablity']}</td>
					<td style='border:1px'><button id='btn_{$i}_{$j}' type='button' onclick=add({$i},{$j})>Add</button></td>
				</tr>
";
			}
			$table.=
